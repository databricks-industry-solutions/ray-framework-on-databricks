...
# MAGIC %pip install -qU ray[all]=2.50.1 xgboost
# MAGIC
...
def train_fn_per_worker(params: dict):
    ...
    dtrain = xgboost.QuantileDMatrix(train_X, label=train_y)
    deval = xgboost.QuantileDMatrix(val_X, label=val_y, ref=dtrain)
...
num_gpu_workers_per_trial = 5
num_hpo_trials = 8
resources = ray.cluster_resources()
total_cluster_gpus = resources.get("GPU") 
max_concurrent_trials = max(1, int(total_cluster_gpus // num_gpu_workers_per_trial))
...
param_space = {
    ...
    "params":{"objective": "multi:softmax",
              ...
              "tree_method": "hist",
              'sampling_method': 'gradient_based', # reduces memory pressure from gradients (default uniform). HIGHLY recommended if using GPU (i.e. device is cuda) + tree_method hist
